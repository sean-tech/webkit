...
func TestGinRbacServer(t *testing.T) {
	...
	}, logging.Logger(), rbac_RegisterApi)
}
...
func rbac_RegisterApi(engine *gin.Engine) {
	...
	apiv1.Use(Authorization(LoadAuthorizationRules))
	{
	...
	rules = []*grbac.Rule{
		&grbac.Rule{
			...
			Resource:   &grbac.Resource{
				Host:   "localhost:8001",
				Path:   "/api/order/v1/rbactest",
				Method: "POST",
			},
			Permission: &grbac.Permission{
				AuthorizedRoles: []string{"superadmin"},
				//ForbiddenRoles:  []string{"normal"},
				AllowAnyone:     false,
			},
		},
	}
